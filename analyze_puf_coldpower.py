...
OUTPUT_DIR = Path("puf_analysis_coldpower")
...
def extract_region_data(filename, region_number):
    """
    Extract one region from the cold-power capture format.
    ...
    region_prefix = "REGION," + str(region_number) + ","
    ...
    if declared_region != region_number:
        raise ValueError(
            "Unexpected region number in "
            + str(filename)
        )

    if declared_size != REGION_SIZE_BYTES:
        raise ValueError(
            "Unexpected region size in "
            + str(filename)
            + ": "
            + str(declared_size)
        )
    ...
    if index + 1 >= len(lines):
        raise ValueError(
            "No hexadecimal data after region metadata in "
            + str(filename)
        )
    ...
    if hex_data.startswith("REGION,"):
        raise ValueError(
            "Missing hexadecimal data for Region "
            + str(region_number)
            + " in "
            + str(filename)
        )
...
def save_statistics_report(
    ...
    with open(
        ...
        file.write(
            ...
            "stability under repeated cold-power captures.\n"
        )
...
def rank_regions(results):
    ...
    print(
        "Best region by measured cold-power stability: "
        + str(best["region"])
    )
    ...
    print(
        ...
        "cold-power intra-device stability only."
    )
...
def main():
    ...
    print()
    print(
        "These are COLD-POWER capture measurements."
    )
