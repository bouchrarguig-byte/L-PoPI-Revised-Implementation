...
def read_region(path: Path, region: int, bytes_per_region: int) -> list[int]:
    lines = [line.strip() for line in path.read_text(encoding="utf-8").splitlines() if line.strip()]
    label = f"REGION {region}"
    try:
        value = lines[lines.index(label) + 1]
    except (ValueError, IndexError) as exc:
        raise ValueError(f"{path}: missing {label}") from exc
    if not re.fullmatch(r"[0-9A-Fa-f]+", value) or len(value) != bytes_per_region * 2:
        raise ValueError(f"{path}: {label} has invalid length or non-hex data")
    return [int(bit) for byte in bytes.fromhex(value) for bit in f"{byte:08b}"]


def load_captures(capture_dir: Path, region: int, bytes_per_region: int) -> list[list[int]]:
    files = sorted(capture_dir.glob("reset_capture_*.txt"))
    if len(files) < 2:
        raise ValueError("At least two reset_capture_*.txt files are required")
