...
import "../src/LPoPIAttestationVerifierFlat.sol";

contract LPoPIAttestationVerifierFlatTest {
    Groth16Verifier verifier;
    LPoPIAttestationVerifierFlat protocol;

    function setUp() public {
        verifier = new Groth16Verifier();
        protocol = new LPoPIAttestationVerifierFlat(address(verifier));
    }
    ...
    function testValidFirstAttestation() public {
        ...
        bytes32 key = protocol.replayKey(input[0], input[1]);

        require(
            protocol.usedAttestation(key),
            "attestation not marked used"
        );
    }
